    ...
    if (language === "english") {
      ...
      await fetch(`https://discord.com/api/v6/users/${botid}`, {
        ...
        .then(data => {
          ...
          ).then((msg) => msg.react("<a:check:850724870282674199>"))
      ...
      message.channel.send("please give a role ID for the bot approver role\ntype `cancel` to cancel").then(() => {
        message.channel.awaitMessages(m => m.author.id === message.author.id, {
          ...
          .then(col => {
            ...
            if (isNaN(msg) || msg.length > 19 || msg.length < 19) return message.lineReply("invalid role ID provided").then(() => {
              db.delete(`setup_running_${message.guild.id}`)
            })
            ...
            message.lineReply("done now please give a bot logging channel id\ntype `cancel` to cancel").then(() => {
              message.channel.awaitMessages(m => m.author.id === message.author.id, {
                ...
                .then(col => {
                  ...
                  if (isNaN(msg) || msg.length > 19 || msg.length < 19) return message.lineReply("invalid channel ID provided").then(() => {
                    db.delete(`setup_running_${message.guild.id}`)
                  })
      ...
      message.channel.send("請回答機器人管理者的身分組ID").then(() => {
        message.channel.awaitMessages(m => m.author.id === message.author.id, {
          ...
          .then(col => {
            ...
            if (isNaN(msg) || msg.length > 19 || msg.length < 19) return message.lineReply("請回答一個正確的身分組ID").then(() => {
              db.delete(`setup_running_${message.guild.id}`)
            })
            db.set(`approverroleid_${message.guild.id}`, msg)
            message.lineReply("請回答機器人日誌的頻道ID\n輸入`cancel`來放棄變更").then(() => {
              message.channel.awaitMessages(m => m.author.id === message.author.id, {
                ...
                .then(col => {
                  ...
                  if (isNaN(msg) || msg.length > 19 || msg.length < 19) return message.lineReply("請回答一個正確的機器人日誌的頻道ID").then(() => {
                    db.delete(`setup_running_${message.guild.id}`)
                  })
                  db.set(`botlogcid_${message.guild.id}`, msg)
